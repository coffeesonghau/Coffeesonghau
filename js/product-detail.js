...
function renderProductDetail() {
  ...
  const galleryHTML = `
        <div class="gallery-container">
            <div class="main-image-frame group">
                <img id="main-product-img" src="${galleryImages[0]}" alt="${
    product.name
  }">
            </div>
  ...
  const relatedProducts = products
    .filter(
      (p) =>
        p.category === product.category && p.id !== product.id && p.price !== 0
    )
    .slice(0, 4);
  ...
  const relatedHTML = relatedProducts
    .map(
      (p) => `
        <div class="bg-white border border-gray-100 p-4 rounded-lg hover:shadow-xl transition-all group cursor-pointer" onclick="window.location.href='product-detail.html?id=${
          p.id
        }'">
            ...
            <h4 class="text-[12px] font-bold text-gray-800 line-clamp-2 h-9 mb-2 uppercase group-hover:text-red-900 transition">${
              p.name
            }</h4>
            ...
            <div class="lg:w-1/2 flex flex-col justify-center">
                <nav class="text-[10px] uppercase tracking-widest text-gray-400 mb-4 flex items-center gap-2">
                    ...
                    <span class="text-red-900 font-bold border-b border-red-900 pb-0.5">
                        ${(() => {
                          if (product.category === "rang-xay")
                            return "Cà phê Rang Xay";
                          if (product.category === "cafe-hat")
                            return "Cà phê Hạt"; // Thêm dòng này
                          if (product.category === "best-seller")
                            return "Cà phê Hòa Tan";
                          if (product.category === "may-pha")
                            return "Máy Pha Cà Phê";
                ...
                <h1 class="text-3xl md:text-4xl font-black text-red-950 mb-3 leading-tight uppercase tracking-tight">${
                  product.name
                }</h1>
                ...
                <div class="flex items-end gap-3 mb-6">
                     ...
                     ${
                       product.price !== 1
                         ? `<span class="text-sm text-gray-400 line-through mb-1.5">Giá thị trường: ${(
                             product.price * 1.2
                ...
                <div class="bg-red-50 p-5 rounded-lg mb-8 border border-red-100 product-description-container">
                    <div id="desc-content" class="desc-content ${
                      product.price === 1 ? "" : "collapsed"
